...
### リモートキャッシュ(ECR)に何を保存するか

CodeBuildのリモートキャッシュとしてECRを利用する場合、キャッシュ用のイメージをどのように管理するかが重要です。
おそらく、変更がほとんどいらないようなベースイメージを管理することがこの機能の主なユースケースになると思います。
...
### CodeBuildがキャッシュを利用しているかどうかの確認方法
...
しかし、実際にビルドがキャッシュを利用しているかどうかをはビルドログを見ないとわからない場合があります。
ビルドログにはキャッシュのヒット率やキャッシュからのプルに関する情報が記載されているため、これを確認することが重要です。
